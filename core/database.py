    ...
    for p in players:
        try:
            c.execute("INSERT OR IGNORE INTO players VALUES (?, ?, ?, ?, ?, ?)", (
                p["name"], p["team"], int(p["matches_played"]), int(p["racks_won"]), int(p["racks_lost"]), float(p["win_percentage"])
            ))
            inserted += c.rowcount
...
def insert_matches(conn, matches):
    ...
    for m in matches:
        try:
            c.execute('''
                INSERT INTO matches (date, team1, team2, player, opponent, player_racks, opponent_racks)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                m["date"], m["team1"], m["team2"], m["player"], m["opponent"], m["player_racks"], m["opponent_racks"]
            ))
            inserted += c.rowcount
        except Exception as e:
